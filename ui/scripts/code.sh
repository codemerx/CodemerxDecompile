...
function code-wsl()
{
	...
	if [ -f "$ELECTRON"  ]; then
		...
		# AGPL
		local WSL_EXT_WLOC=$(echo "" | VSCODE_DEV=1 ELECTRON_RUN_AS_NODE=1 "$ROOT/.build/electron/CodemerxDecompile.exe" "out/cli.js" --ms-enable-electron-run-as-node --locate-extension $WSL_EXT_ID)
		# End AGPL
		cd $CWD
