...
namespace Mix.Cms.Lib.Controllers
{
	...
	where TView : ViewModelBase<TDbContext, TModel, TView>
	{
		...
		{
			ReflectionHelper.SetPropertyValue<TView>(data, new JProperty("CreatedBy", base.get_User().Identity.Name));
			ReflectionHelper.SetPropertyValue<TView>(data, new JProperty("CreatedDateTime", (object)DateTime.UtcNow));
		...
		{
			RepositoryResponse<List<TModel>> modelListByAsync = await DefaultModelRepository<TDbContext, TModel>.get_Instance().GetModelListByAsync(predicate, BaseRestApiController<TDbContext, TModel, TView>._context, null);
			...
			UnitOfWorkHelper<TDbContext>.HandleTransaction(modelListByAsync.get_IsSucceed(), true, BaseRestApiController<TDbContext, TModel, TView>._transaction);
		...
		public virtual async Task<ActionResult<PaginationModel<TView>>> Get()
		{
			...
			nullable = null;
			TDbContext tDbContext = default(TDbContext);
			ConfiguredTaskAwaitable<RepositoryResponse<PaginationModel<TView>>> configuredTaskAwaitable = DefaultRepository<TDbContext, TModel, TView>.get_Instance().GetModelListAsync(requestPaging1.get_OrderBy(), requestPaging1.get_Direction(), requestPaging1.get_PageSize(), new int?(requestPaging1.get_PageIndex()), nullable1, nullable, tDbContext, null).ConfigureAwait(false);
			RepositoryResponse<PaginationModel<TView>> repositoryResponse = await configuredTaskAwaitable;
			return (!repositoryResponse.get_IsSucceed() ? this.BadRequest(repositoryResponse.get_Errors()) : this.Ok(repositoryResponse.get_Data()));
		}
		...
		protected async Task<RepositoryResponse<PaginationModel<TView>>> GetListAsync(Expression<Func<TModel, bool>> predicate = null)
		{
			...
			Enum.TryParse<MixHeartEnums.DisplayDirection>(base.get_Request().get_Query().get_Item("direction"), out displayDirection);
			bool flag = int.TryParse(base.get_Request().get_Query().get_Item("pageSize"), out num1);
			Enum.TryParse<MixEnums.MixContentStatus>(base.get_Request().get_Query().get_Item("status"), out mixContentStatu);
			RequestPaging requestPaging = new RequestPaging();
			requestPaging.set_PageIndex(num);
			num2 = (flag ? num1 : 100);
			requestPaging.set_PageSize(new int?(num2));
			...
			requestPaging.set_Direction(displayDirection);
			RequestPaging requestPaging1 = requestPaging;
			RepositoryResponse<PaginationModel<TView>> modelListByAsync = null;
			if (modelListByAsync == null)
			{
				...
				else
				{
					...
					nullable = null;
					tDbContext = default(TDbContext);
					modelListByAsync = await DefaultRepository<TDbContext, TModel, TView>.get_Instance().GetModelListByAsync(predicate, requestPaging1.get_OrderBy(), requestPaging1.get_Direction(), requestPaging1.get_PageSize(), new int?(requestPaging1.get_PageIndex()), nullable2, nullable, tDbContext, null);
				}
			}
		...
		public async Task<IActionResult> Patch(string id, [FromBody] JObject fields)
		{
			...
			{
				ReflectionHelper.SetPropertyValue<TView>(singleAsync.get_Data(), new JProperty("ModifiedBy", base.get_User().Identity.Name));
				ReflectionHelper.SetPropertyValue<TView>(singleAsync.get_Data(), new JProperty("LastModified", (object)DateTime.UtcNow));
		...
		protected RepositoryResponse<List<TView>> SaveList(List<TView> lstVm, bool isSaveSubModel)
		{
			...
			{
				RepositoryResponse<TView> repositoryResponse2 = tView.SaveModel(isSaveSubModel, BaseRestApiController<TDbContext, TModel, TView>._context, BaseRestApiController<TDbContext, TModel, TView>._transaction);
		...
		public async Task<IActionResult> Update(string id, [FromBody] TView data)
		{
			...
			else
			{
				...
				if (id == str)
				{
					ReflectionHelper.SetPropertyValue<TView>(data, new JProperty("ModifiedBy", base.get_User().Identity.Name));
					ReflectionHelper.SetPropertyValue<TView>(data, new JProperty("LastModified", (object)DateTime.UtcNow));
					RepositoryResponse<TView> repositoryResponse = await this.SaveAsync(data, true);
					if (repositoryResponse.get_IsSucceed())
					{
