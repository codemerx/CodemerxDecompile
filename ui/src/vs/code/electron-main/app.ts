...
export class CodeApplication extends Disposable {
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		/* AGPL */
		const grpcChannel = ProxyChannel.fromService(accessor.get(IGrpcMainService), disposables);
		mainProcessElectronServer.registerChannel('grpc', grpcChannel);

		const decompilationChannel = ProxyChannel.fromService(accessor.get(IDecompilationMainService), disposables);
		mainProcessElectronServer.registerChannel('decompilationService', decompilationChannel);

		const analyticsChannel = ProxyChannel.fromService(accessor.get(IAnalyticsMainService), disposables);
		mainProcessElectronServer.registerChannel('analyticsService', analyticsChannel);
		/* End AGPL */
	}
