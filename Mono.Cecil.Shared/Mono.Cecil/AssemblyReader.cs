...
namespace Mono.Cecil {
	...
	sealed class MetadataReader : ByteBuffer {
		...
		FieldDefinition ReadField (uint field_rid)
		{
			...
			/*Telerik Authorship*/
			TypeReference type = ReadFieldType(signature);
			if (type == null)
				return null;
		...
		public ConstantValue ReadConstant (IConstantProvider owner)
		{
			...
			case ElementType.String:
				/*Telerik Authorship*/
				return new ConstantValue(ReadConstantString (ReadBlob (row.Col2)), row.Col1);
			default:
				/*Telerik Authorship*/
				return new ConstantValue(ReadConstantPrimitive (row.Col1, row.Col2), row.Col1);
			}
		}
