...
namespace CodemerxDecompile.Service.Services.Search
{
    public class SearchService : ISearchService
    {
        ...
        public IEnumerable<SearchResult> Search(AssemblyDefinition targetAssembly, string searchString, bool matchCase = false)
        {
            this.cachedSearchResults = new Dictionary<int, SearchResult>();

            IEnumerable<TypeDefinition> types = targetAssembly.Modules.SelectMany(m => m.GetTypes());
        ...
        {
            if (this.decompilationContext.TryGetTypeFilePathFromCache(declaringType, out string typeFilePath))
            {
